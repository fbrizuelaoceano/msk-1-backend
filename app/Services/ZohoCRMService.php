...
class ZohoCRMService
{
    ...
    public function Update($module, $requestArray, $id)
    {
            ...
            if(isset($response["code"])){
                ...
                //     "status": "error"
                // }
                if($response["code"]==="INVALID_URL_PATTERN"){
                    // Lanza una excepción personalizada con los datos en un campo específico
                    throw new \Exception("Error en la url: ". $URL_ZOHO, 500, new \Exception(json_encode($body)));
                }
            }
            if(isset($response["data"][0])){
                // enity_id_crm que no existe en crm /54621354
                ...
                if(isset($response["data"][0]["code"])){
                   if($response["data"][0]["code"]==="INVALID_DATA"){
                       // Lanza una excepción personalizada con los datos en un campo específico
                       throw new \Exception("Error, data invalida: ". json_encode($requestArray). " Url: ". $URL_ZOHO, 500, new \Exception(json_encode($body)));
                   }
               }
            }
